...
# 命令行参数解析
###############################################
parser <- ArgumentParser(description = "Differential methylation analysis (limma + DMRcate)。example: Rscript script.R -i methylation_matrix.txt -g Control,Control,Case,Case -o result_dir")
...
if (!is.null(args$cell_props)) {
  ...
  if (!all(colnames(beta) %in% rownames(cell))) {
    stop("细胞比例文件的行名必须与 beta 矩阵的列名（样本名）完全匹配！\n",
         "缺失的样本: ", paste(setdiff(colnames(beta), rownames(cell)), collapse = ", "))
  }
  ...
  if (any(is.na(cell_mat))) {
    stop("细胞比例文件中存在 NA，请先处理！")
  }
  
  # 合并到设计矩阵
  design <- cbind(design, cell_mat)
  
  cat("已加入细胞比例协变量，共", ncol(cell_mat), "种细胞类型:\n")
  print(colnames(cell_mat))
  cat("设计矩阵维度:", paste(dim(design), collapse = " x "), "\n")
} else {
  cat("\n未提供细胞比例文件，不进行细胞组成矫正。\n")
}
...
# 计算 deltaBeta（注意：这里仍用原始 beta 计算，未矫正细胞比例后的效应量）
